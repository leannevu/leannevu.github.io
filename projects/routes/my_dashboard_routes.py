...
def load_local_launch_env():
    ...
    # Support both the original standalone layout and the combined portfolio.
    # Real deployment variables still win because load_env_lines never
    # overwrites a value already supplied by the process environment.
    app_root = PROJECT_ROOT.parent if PROJECT_ROOT.name == "projects" else PROJECT_ROOT
    env_files = [app_root / ".env"]
...
def api_runtime_error_response(exc):
    if request.path.startswith("/projects/study-vault/api/"):
...
def require_oauth_login():
    ...
    public_endpoints = {
        "newdashboard.login",
        "newdashboard.google_login",
        "newdashboard.google_callback",
        "newdashboard.static",
    }
    if request.endpoint in public_endpoints:
        return None
    if request.path.startswith("/projects/study-vault/study-dashboard-static/"):
        return None
    if current_user():
        return None
    if request.path.startswith("/projects/study-vault/api/"):
